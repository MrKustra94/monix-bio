...
object BIO extends TaskInstancesLevel0 {
  ...
  /** Create a cancelable `Task` from an asynchronous computation,
    ...
    *
    * @see [[BIO.cancelable[E,A](register* Task.cancelable]] for the simpler
   ...
   * }}}
   *
   * See [[BIO.parMap3]] for parallel processing.
   */
   ...
   * }}}
   *
   * See [[BIO.parMap4]] for parallel processing.
   */
   ...
   * }}}
   *
   * See [[BIO.parMap5]] for parallel processing.
   */
   ...
   * }}}
   *
   * See [[BIO.parMap6]] for parallel processing.
   */
   ...
   *
   * This is a specialized [[BIO.gather]] operation and as such
   * the BIOs are evaluated in parallel, ordering the results.
   * In case one of the BIOs fails, then all other BIOs get
   ...
   *
   *   // Yields Failure(e), because the second arg is a failure
   *   BIO.parMap2(fa1, BIO.raiseError(new RuntimeException("boo"))) { (a, b) =>
   *     a + b
   *   }
   * }}}
   *
   ...
   *
   *   // Yields Failure(e), because the second arg is a failure
   *   BIO.parMap3(fa1, BIO.raiseError(new RuntimeException("boo")), fa3) { (a, b, c) =>
   *     a + b + c
   *   }
   * }}}
   *
  ...
  /** Pairs 4 `BIO` values, applying the given mapping function,
   * ordering the results, but not the side effects, the evaluation
   * being done in parallel if the BIOs are async.
   *
   * This is a specialized [[BIO.gather]] operation and as such
   * the BIOs are evaluated in parallel, ordering the results.
   * In case one of the BIOs fails, then all other BIOs get
   ...
   *
   *   // Yields Failure(e), because the second arg is a failure
   *   BIO.parMap4(fa1, BIO.raiseError(new RuntimeException("boo")), fa3, fa4) {
   *     (a, b, c, d) => a + b + c + d
   *   }
   * }}}
   *
   ...
   *
   *   // Yields Failure(e), because the second arg is a failure
   *   BIO.parMap5(fa1, BIO.raiseError(new RuntimeException("boo")), fa3, fa4, fa5) {
   *     (a, b, c, d, e) => a + b + c + d + e
   *   }
   * }}}
   *
   ...
   */
  def parMap5[E, A1, A2, A3, A4, A5, R](fa1: BIO[E, A1], fa2: BIO[E, A2], fa3: BIO[E, A3], fa4: BIO[E, A4], fa5: BIO[E, A5])(
    f: (A1, A2, A3, A4, A5) => R): BIO[E, R] = {
   ...
   *
   *   // Yields Failure(e), because the second arg is a failure
   *   BIO.parMap6(fa1, BIO.raiseError(new RuntimeException("boo")), fa3, fa4, fa5, fa6) {
  ...
  def parZip4[E, A1, A2, A3, A4](
      ...
      fa2: BIO[E, A2],
      fa3: BIO[E, A3],
      fa4: BIO[E, A4]): BIO[E, (A1, A2, A3, A4)] =
  ...
  def parZip5[E, A1, A2, A3, A4, A5](
      ...
      fa3: BIO[E, A3],
      fa4: BIO[E, A4],
      fa5: BIO[E, A5]): BIO[E, (A1, A2, A3, A4, A5)] =
  ...
  def parZip6[E, A1, A2, A3, A4, A5, A6](
      ...
      fa4: BIO[E, A4],
      fa5: BIO[E, A5],
      fa6: BIO[E, A6]): BIO[E, (A1, A2, A3, A4, A5, A6)] =
